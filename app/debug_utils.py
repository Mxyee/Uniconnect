...
def reset_db():
    ...
    a2 = Assignment(title='Physics Lab', description='Submit lab report', deadline=datetime.strptime('2025-05-12', '%Y-%m-%d'), professor_id=prof2.id)
    a3 = Assignment(title='Programming Project', description='Build a Flask app', deadline=datetime.strptime('2025-05-15', '%Y-%m-%d'), professor_id=prof1.id)
    ...
    # Activities
    act1 = Activity(title='AI Study Group', description='Discuss ML topics', date=datetime(2025, 4, 25, 15, 30), location='Room 101', created_by=stu1.id)
    act2 = Activity(title='Startup Club Meetup', description='Pitch ideas and get feedback', date=datetime(2025, 4, 27, 10, 0), location='Library Hall', created_by=stu2.id)
    ...
    # Notifications
    n1 = Notification(user_id=stu1.id, message='Assignment Math Homework 1 is due soon!', date=datetime.strptime('2025-04-20', '%Y-%m-%d'))
    n2 = Notification(user_id=stu3.id, message='You have joined AI Study Group.', date=datetime.strptime('2025-04-22', '%Y-%m-%d'))
    n3 = Notification(user_id=prof1.id, message='New submission received for Programming Project.', date=datetime.strptime('2025-04-23', '%Y-%m-%d'))
