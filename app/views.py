...
def my_assignments():
    ...
    return render_template("my_assignments.html", title='My Assignment', assignments=assignments)
...
def submit_assignment(assignment_id):
    ...
    submission = db.session.scalar(
        db.select(Submission).where((Submission.assignment_id == assignment.id) & (Submission.student_id==current_user.id)))
    form = SubmissionForm()
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Your submission has been submitted', 'success')
        return redirect(url_for('my_assignments'))
    if submission:
        form.content.data = submission.content

    return render_template('submission_form.html',title='submission_form' , assignment=assignment, form=form, submission=submission)
